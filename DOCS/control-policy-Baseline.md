...
- 범위:
  ...
  - `hmi_action`: 경고 메시지 (EOR/HOR/DCA/MRM)
...
- `dashboard_state` (권장):
  ...
  - `hmi_action`: EOR/HOR/DCA/MRM 경고/지시 메시지
...
| WARNING | `<= 0.60 * lkas_throttle` | EOR/HOR (반복 음향/햅틱) | false |
...
- `lkas_mode` (`OFF/ON_INACTIVE/ON_PASSIVE/ON_ACTIVE`)
- `manoeuvre_type` (`NONE/CURVE_FOLLOW/LANE_CHANGE/TURN/MRM`)
- `hmi_action` (상태별 경고: EOR/HOR/DCA/MRM)
...
def evaluate_policy(
    ...
    # 상태별 기본값
    policy_map = {
        DriverState.OK: (1.00, "정보성", False),
        DriverState.WARNING: (0.60, "EOR/HOR", False),
        DriverState.UNRESPONSIVE: (0.20, "DCA", False),
        DriverState.ABSENT: (0.0, "MRM", True),
    }
    
    ratio, hmi, mrm_active = policy_map.get(driver_state, (1.0, "정보성", False))
    
    # stale 처리
    if input_stale:
        throttle_limit = 0.0
    else:
        throttle_limit = ratio * lkas_throttle
...
- **stale 처리**: `input_stale=true`일 때 `throttle_limit=0.0`
